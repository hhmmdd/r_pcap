...
/* calculate characteristic value of a message -- this value
   ...
   reply matching
 */
__inline static int char_msg_val( struct sip_msg *msg, char *cv )
{
	str src[8];
 ...
 */
__inline static char* get_body(struct sip_msg *msg)
{
	int offset;
	int len;
	...
	if ((len+2 <= (int) msg->len) && (strncmp(CRLF,msg->unparsed,CRLF_LEN)==0) )
		offset = CRLF_LEN;
	else if ( (len+1<= (int)msg->len) &&
				(*(msg->unparsed)=='\n' || *(msg->unparsed)=='\r' ) )
		offset = 1;
	else
		return 0;
