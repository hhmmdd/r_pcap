...
normal_exit:
...
#ifdef SIP_DEBUG
	printf("Found param type %d, <%.*s> = <%.*s>; state=%d\n", param->type,
 ...
 end_via:
	...
	*pstate=state;
	*psaved_state=saved_state;
	printf("Error on  param type %d, <%.*s>, state=%d, saved_state=%d\n",
...
static __inline void free_via_param_list(struct via_param* vp)
{
