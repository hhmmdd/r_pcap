...
__inline static char* eat_space_end(char* p, char* pend)
{
	for(;(p < pend)&&(*p == ' ' || *p=='\t') ;p++);
	return p;
}
...
__inline static char* eat_lws_end(char* p, char* pend)
{
	while(p<pend) {
		if (SP(*p)) p++;
...
__inline static char* eat_token_end(char* p, char* pend)
{
	for (;(p<pend)&&(*p!=' ')&&(*p!='\t')&&(*p!='\n')&&(*p!='\r'); p++);
	return p;
}
...
__inline static char* eat_token2_end(char* p, char* pend, char delim)
{
...
__inline static int is_empty_end(char* p, char* pend )
{
 ...
 */
__inline static char* find_not_quoted(str* _s, char _c)
{
	int quoted = 0, i;
