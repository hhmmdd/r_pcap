...
void get_sip_contact_port(struct sip_msg *msg,char *port)
{
   //char sip_contact_port[10] ="";
   char *ptr = NULL;
   char *tmp = NULL;
   int offset = 0;
   int remain_len = 0;
   
  // tmp = msg->h_via1->body.s;
  ...
  // if (tmp == NULL) return;
   ...
   do{
	   tmp++;
	   offset++;
   }while(offset < msg->h_via1->body.len && *tmp != ':');


   if ( offset < msg->h_via1->body.len ){
	   tmp++;
	   remain_len = msg->h_via1->body.len - offset;
	   //strncpy(port,tmp,remain_len);
	   ptr = tmp;
       offset = 0;
	   do{
		   offset++;
		   tmp++;
	   }while ( offset < remain_len && *tmp != ';');
       strncpy(port,ptr,offset);
	   port[offset] ='\0';
	   return;
   }else{
  */
	//   tmp = NULL;
	   ...
	   if (msg->contact == NULL)return;
	   tmp= msg->contact->body.s;
	   ...
	   do{
		   tmp++;
		   offset++;
	   }while(tmp != NULL && *tmp != '>'&& offset < msg->contact->body.len );
