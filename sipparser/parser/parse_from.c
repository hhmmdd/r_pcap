...
int parse_to_header( struct sip_msg *msg)
{
	...
	if ( !msg->to && ( parse_headers(msg,HDR_TO,0)==-1 || !msg->to)) {
		LOG(L_ERR,"ERROR:parse_to_header: bad msg or missing TO header\n");
		goto error;
	}
	...
	if (to_b == 0) {
		LOG(L_ERR, "ERROR:parse_to_header: out of pkg_memory\n");
		goto error;
	}
	...
	if (to_b->error == PARSE_ERROR) {
		LOG(L_ERR, "ERROR:parse_to_header: bad to header\n");
		pkg_free(to_b);
		goto error;
	}
	msg->to->parsed = to_b;
