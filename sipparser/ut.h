 ...
 * error and if err!=null
  */
static __inline unsigned short str2s(char* s, unsigned int len,
									int *err)
{
	unsigned short ret;
	int i;
...
error_digits:
	printf("str2s: ERROR: too many letters in [%.*s]\n", (int)len, init);
	if (err) *err=1;
	return 0;
error_char:
	printf("str2s: ERROR: unexpected char %c in %.*s\n", *str, (int)len, init);
	if (err) *err=1;
	return 0;
}
...
static __inline int btostr( char *p,  unsigned char val)
{
...
static __inline char* int2str(unsigned int l, int* len)
{
...
static __inline char* q_memchr(char* p, int c, unsigned int size)
{
	char* end;
...
__inline static int reverse_hex2int( char *c, int len )
{
	char *pc;
	int r;
	char mychar;
...
__inline static int int2reverse_hex( char **c, int *size, int nr )
{
...
__inline static int string2hex( 
...
__inline static void sleep_us( unsigned int nusecs )
{
	/*
	struct timeval tval;
...
__inline static int hex2int(char hex_digit)
{
...
*/
__inline static int un_escape(str *user, str *new_user ) 
{
 	int i, j, value;
	int hi, lo;
 ...
 */
static __inline void strlower(str* _s)
{
	int i;
 ...
 */
static __inline int str2int(str* _s, unsigned int* _r)
{
	int i;
 ...
 */
static __inline int str2float(str* _s, float* _r)
{
	int i, dot = 0;
	float order =(float) 0.1;
