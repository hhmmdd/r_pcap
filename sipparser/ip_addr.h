...
/* returns 1 if ip & net.mask == net.ip ; 0 otherwise & -1 on error 
	[ diff. adress fams ]) */
static __inline int matchnet(struct ip_addr* ip, struct net* net)
{
	unsigned int r;
	int ret;
...
static __inline void sockaddr2ip_addr(struct ip_addr* ip, struct sockaddr* sa)
{
	switch(sa->sa_family){
	case AF_INET:
			ip->af=AF_INET;
...
static __inline int su_cmp(union sockaddr_union* s1, union sockaddr_union* s2)
{
...
static __inline short su_getport(union sockaddr_union* su)
{
...
static __inline void su_setport(union sockaddr_union* su, unsigned short port)
{
	switch(su->s.sa_family){
		case AF_INET:
...
static __inline void su2ip_addr(struct ip_addr* ip, union sockaddr_union* su)
{
	switch(su->s.sa_family){
	case AF_INET: 
					ip->af=AF_INET;
...
static __inline int init_su( union sockaddr_union* su,
							struct ip_addr* ip,
							unsigned short   port ) 
{
 ...
 * returns 0 if ok, -1 on error (unknown address family) */
static __inline int hostent2su( union sockaddr_union* su,
								struct hostent* he,
								unsigned int idx,
								unsigned short   port ) 
{
 ...
 */
static __inline char* ip_addr2a(struct ip_addr* ip)
{
 ...
 * statical structure */
static __inline struct hostent* ip_addr2he(str* name, struct ip_addr* ip)
{
